...
export default function GameDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSyncSteam = async () => {
    ...
    try {
      // 1. Sync Playtime
      const gamesRes = await fetch(`/api/steam/games?steamid=${userData.steamId}`);
      const gamesData = await gamesRes.json();
      const steamGame = gamesData.response?.games?.find((g: any) => g.appid === game.steam_appid);

      let newHours = game.horas_jugadas || 0;
      if (steamGame) {
        newHours = Math.floor(steamGame.playtime_forever / 60);
      }

      // 2. Sync Achievements
      const achRes = await fetch(`/api/steam/achievements?steamid=${userData.steamId}&appid=${game.steam_appid}`);
      ...
      if (achData.error === "PRIVACY_ERROR") {
        ...
        setSyncError(null);
        if (achData.total > 0) {
          newProgress = achData.percentage;
        }
      }
      ...
      if (!achData.error && achData.total > 0) {
        updateData.progreso = newProgress;
      }

      await updateDoc(doc(db, "usuario_juego", game.id!), updateData);
      
      setGame({ 
        ...game, 
        horas_jugadas: newHours, 
        progreso: updateData.progreso || game.progreso 
      });

    } catch (error: any) {
      console.error("Error syncing with steam:", error);
      setSyncError("Error al sincronizar con Steam. Revisa tu conexión.");
    } finally {
      setSyncing(false);
    }
  };
    ...
    <div className="flex flex-col gap-6 font-sans">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {syncError && (
             <div className="hidden lg:block text-xs text-rose-400 bg-rose-500/10 border border-rose-500/20 px-3 py-1.5 rounded-lg animate-pulse mr-2">
                {syncError}
             </div>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          {game.motivo_estado && (game.estado === 'pausado' || game.estado === 'abandonado') && (
            <div className={`p-4 rounded-xl border flex gap-4 items-start ${game.estado === 'pausado'
                ? 'bg-amber-500/10 border-amber-500/20 text-amber-200'
                : 'bg-rose-500/10 border-rose-500/20 text-rose-200'
              }`}>
              <div className={`mt-1 p-1.5 rounded-lg ${game.estado === 'pausado' ? 'bg-amber-500/20' : 'bg-rose-500/20'
                }`}>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex flex-col items-end">
                  ...
                  {game.steam_appid && (
                    <span className="text-[10px] text-soft opacity-70 uppercase tracking-tighter flex items-center gap-1">
                      <Trophy className="w-2.5 h-2.5" /> Basado en logros de Steam
                    </span>
                  )}
                </div>
              </div>
